  ...
  <div class="box-header with-border">
    ...
    <div class="box-tools">
      <div class="btn-group float-right" style="margin-right: 10px">
        <a href="{{ admin_route('orders.index') }}" class="btn btn-sm btn-default"><i class="fa fa-list"></i> 列表</a>
      </div>
    </div>
  </div>
  <div class="box-body">
    <table class="table table-bordered">
      ...
      @if($order->ship_status === \App\Models\Order::SHIP_STATUS_PENDING)
       @if($order->refund_status !== \App\Models\Order::REFUND_STATUS_SUCCESS && ($order->type !== \App\Models\Order::TYPE_CROWDFUNDING || $order->item[0]->product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_SUCCESS))
      <tr>
        <td colspan="4">
          <form action="{{ admin_route('orders.ship', [$order->id]) }}" method="post" class="form-inline">
            <!-- 别忘了 csrf token 字段 -->
            {{ csrf_field() }}
  ...
  $('#btn-refund-disagree').click(function() {
    ...
    Swal.fire({
      ...
      preConfirm: function(inputValue) {
        ...
        return $.ajax({
          url: '{{ admin_route('orders.handle_refund', [$order->id]) }}',
          type: 'POST',
